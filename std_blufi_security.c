...
void blufi_dh_negotiate_data_handler(uint8_t *data, int len, uint8_t **output_data, int *output_len, bool *need_free)
{
    ...
    if (blufi_sec == NULL) {
        STD_LOGE("BLUFI Security is not initialized");
        return;
    }
    ...
    case SEC_TYPE_DH_PARAM_LEN:
        ...
        if (blufi_sec->dh_param == NULL) {
            STD_LOGE("%s, malloc failed\n", __func__);
            return;
        }
        break;
    case SEC_TYPE_DH_PARAM_DATA:{
        if (blufi_sec->dh_param == NULL) {
            STD_LOGE("%s, blufi_sec->dh_param == NULL\n", __func__);
            return;
        }
        ...
        if (ret) {
            STD_LOGE("%s read param failed %d\n", __func__, ret);
            return;
        }
        free(blufi_sec->dh_param);
        blufi_sec->dh_param = NULL;
        ...
        if (ret) {
            STD_LOGE("%s make public failed %d\n", __func__, ret);
            return;
        }
